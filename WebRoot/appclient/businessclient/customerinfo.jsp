...
<html>
	<head>
		...
		<script type="text/javascript" src="${ctx}/appclient/businessclient/js/zoom.js"></script>
		<script type="text/javascript" src="${ctx}/appclient/businessclient/js/zepto.js"></script>
	    <script type="text/javascript" src="${ctx}/appclient/businessclient/js/touch.js"></script>
	</head>
	<body>
		<header id="header">
		<div class="header-m">
	...
	<div id="main">
		<section class="gongdan">
			<ul class="kh-list">
				<li>
					...
					<p><span>到期时间：<fmt:formatDate value='${customerInfoMap.expiry_date}' pattern='yyyy-MM-dd' /></span></p>
				</li>
			</ul>
		</section>
		...
		<section class="box">
			...
			<div class="hm">
				...
				<p>签约时间：<fmt:formatDate value='${contracts.signing_time}' pattern='yyyy-MM-dd' /></p>
				<p>合作金额：${contracts.cooperation_num}</p>
				<div class="ht-img">
					<c:forEach items="${contracts.pictureList}" var="picture">
					<span><img src="${ctx}/fileupload/${picture.path}" /><em>${picture.sort}</em></span>
					</c:forEach>
				</div>
			...
			<c:forEach items="${contracts.workOrderList}" var="workOrder">
				<ul class="xm-list">
					<li>
						<a href="${ctx}/business.action?methType=workOrder&flag=0&version=1&workOrderId=${workOrder.work_order_id}">
							<h5><span class="xm-sb left">[${workOrder.typeName}]</span><span class="xm-tit left">${workOrder.buy_items}</span><span class="xm-jc right">查看进程</span></h5>
							<p><span>单价 ${workOrder.amount}元</span><span>服务年限 ${workOrder.years}年</span></p>
							<p><span>于<fmt:formatDate value='${workOrder.expiry_date}' pattern='yyyy-MM-dd' />到期</span></p>
						</a>
					</li>
				</ul>
			</c:forEach>
