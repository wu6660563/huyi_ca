...
function EV_modeAlert1(msgID, type){  
    //创建大大的背景框  
	var bgObj = document.createElement("div");  
    bgObj.setAttribute('id','EV_bgModeAlertDiv');
   	document.body.appendChild(bgObj);  
    ...
    EV_Show_bgDiv();  
   	...
   	if(type == 'CUSTOMERINFO') {
   		$("#importTitle").html("导入客户");
   	} else if(type == 'CONTRACT') {
   		$("#importTitle").html("导入合同");
   	} else if(type == 'ORDER') {
   		$("#importTitle").html("导入工单");
   	} else if(type == 'PROCESS') {
   		$("#importTitle").html("导入服务进程");
   	} else if(type == 'QUESSTION') {
   		$("#importTitle").html("导入问题单");
   	} else if(type == 'EMPLOYEE') {
   		$("#importTitle").html("导入用户");
   	}
}  
