...
<script type="text/javascript">
       ...
       //实例化编辑器
	    ...
	    UE.getEditor('editor');
	      ...
	      //保存
		  function update_info_diss(){
		    $("#context").val(UE.getEditor('editor').getContent());
			   $('#update_info_diss').submit();
			  };
	  //取消
		 function back(){
			 //取消之前添加的图片
...
<body>
	<form action="${ctx}/home.action" id="update_info_diss" method="post">
	<input type="hidden" name="operType" value="update_info_diss">
	...
	<div id="main">
		<div class="custom">
			<h4 class="hd"><span class="title">修改业务资讯</span><div class="hd-r"><a href="javascript:void(0);" onclick="back();" class="back-btn">返回</a></div></h4>
			<div class="hm">
				<input type="hidden" name="info_id" value="${info_dissMap.info_id }">
				 <div class="addYw">
					 <p><label>标题：</label><input type="text" name="title" value="${info_dissMap.title }"/></p>
					 <p><label>资讯类别：</label>
					...
					<p class="addImg">
						...
						<span class="uploadImg" id="showImgDiv">
								<img src="${ctx}/fileupload/${info_dissMap.path}" id="addImg" />
								<input type='hidden' value='${info_dissMap.path }' name='upImg' id='upImg'/>
								<input type='hidden' value='${info_dissMap.or_path }' name='old_filePath' id='old_filePath'/>
						</span>
						<a href="javascript:void(0);" class="loadBtn" id="upload-btn" >上传图片</a>
					</p>
					 <p><label>摘要：</label><textarea class="zy-txt" name="summary" >${info_dissMap.summary}</textarea></p>
					 
				 </div>
				 <div class="editorArea">
			<h4>图文编辑</h4>
		     	<input type="hidden" id="context" name="context"/>
		     	<textarea cols="80" id="editor" name="editor" rows="10" style="height: 300px;">${info_dissMap.context}</textarea>
		   </div>
				 <div class="textcenter f-btn"><a href="javascript:void(0);" onclick="back();" class="b-cannel">取消</a><a onclick="update_info_diss();" href="javascript:void(0);" class="b-save">添加</a></div>
			</div>
		</div>
	</div><!--main-->
